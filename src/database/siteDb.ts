...
import { getDynamicGalleryPhotos, getDynamicHeroImages } from '@/services/galleryService'
import { INITIAL_PACKAGES } from '@/utils/packagesData'
...
export async function ensureInitialData(force = false) {
  ...
  try {
    ...
    // Remove obsolete placeholder photos, old /gallery/ paths, and removed duplicate photos
    await db.galleryPhoto.deleteMany({
      where: {
        OR: [
          { category: { in: ['commercial', 'event'] } },
          { src: { startsWith: '/gallery/' } },
          { src: { contains: 'wedding1.jpg' } },
          { src: { contains: 'wedding2.jpg' } },
          { src: { contains: 'portrait1.jpg' } },
          { src: { contains: 'portrait2.jpg' } },
          { src: { contains: 'IMG_7952.JPG.jpeg' } },
          { src: { contains: 'bridetobe1_original_wide' } },
          { src: { endsWith: 'D93A9617.jpg' } },
          { src: { endsWith: 'DSC03954.jpg' } },
        ],
      },
    })
...
export async function getFullSiteData() {
  ...
  const getBaseKey = (src: string) => {
    try {
      const decoded = decodeURIComponent(src)
      return path
        .basename(decoded)
        .toLowerCase()
        .replace(/\.(jpe?g|png|webp|avif)(\.jpe?g)?$/i, '')
        .replace(/[-_]+/g, '')
    } catch {
  ...
  // Add dynamic photos first (guaranteeing one entry per unique photo)
  for (let idx = 0; idx < dynamicPhotos.length; idx++) {
    const dp = dynamicPhotos[idx]
    const baseKey = getBaseKey(dp.src)
    if (seenBaseFilenames.has(baseKey)) continue
    seenBaseFilenames.add(baseKey)

    const dbItem = gallery.find((g) => getBaseKey(g.src) === baseKey)
    mergedGallery.push({
      id: dbItem?.id || `folder-${idx}`,
      src: dp.src,
      title: dp.title, // clean short moment title
  ...
  // Append any extra unique uploads (e.g. from admin) not in dynamic folders
  for (const dbItem of gallery) {
    const baseKey = getBaseKey(dbItem.src)
    if (!seenBaseFilenames.has(baseKey) && !dbItem.src.startsWith('/gallery/') && !dbItem.src.includes('wedding1.jpg')) {
      seenBaseFilenames.add(baseKey)
      mergedGallery.push({
        id: dbItem.id,
        src: dbItem.src,
